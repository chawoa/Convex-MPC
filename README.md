...
방정식 28: 목표 궤적($y$, 논문에서는 $x_{\text{ref}}$)과 실제 궤적($X = A_{qp}x_0 + B_{qp}U$)의 차이를 줄이고, 힘($U$)도 최소한으로 쓰겠다는 목적 함수입니다.$$J(U) = ||A_{qp}x_0 + B_{qp}U - y||_L^2 + ||U||_K^2$$  
$U^2$ 항 (2차항): $U^\top (B_{qp}^\top L B_{qp} + K) U$  
$U^1$ 항 (1차항): $U^\top (2 B_{qp}^\top L (A_{qp}x_0 - y))$  
$U$가 없는 항 (상수항): $(A_{qp}x_0 - y)^\top L (A_{qp}x_0 - y)$  

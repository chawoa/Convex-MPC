...
피드백(대괄호): 목표 위치/속도($\text{ref}$)와 현재 위치/속도의 차이(오차)를 계산, 여기서 K_p는 강성, K_d는 감쇠력
...
- 각도 변화: $\dot{R} = [\omega]_\times R$
    ...
    $[\omega]_\times$ (외적 행렬, Skew-symmetric matrix): 각속도 벡터 $\omega = [w_x, w_y, w_z]^\top$를 $3 \times 3$ 행렬 모양으로 변환한 행렬
...
$G$: 크기가 $12 \times 1$ 인 중력 벡터 $\begin{bmatrix} 0 \\ 0 \\ 0 \\ g \end{bmatrix}$ (마지막 3칸만 중력 가속도 값을 가짐)
